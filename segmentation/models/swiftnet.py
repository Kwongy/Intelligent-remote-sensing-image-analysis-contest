...
from .util import _Upsample, _BNReluConv, SpatialPyramidPooling
...
class ResNet(nn.Module):
    def __init__(self, block, layers, *, num_features=128, k_up=3, efficient=True, use_bn=True,
                 spp_grids=(8, 4, 2, 1), spp_square_grid=False, num_classes=19, **kwargs):
    ...
    def forward(self, image):
        return nn.functional.upsample(self.forward_up(self.forward_down(image)), scale_factor=4, mode="bilinear",
                                      align_corners=False)
...
class swiftnet(nn.Module):
    ...
    def __init__(self, pretrained=True, num_classes=16, num_features=128, use_bn=True):
        super(swiftnet, self).__init__()
        self.backbone = resnet18(pretrained=pretrained, num_classes=num_classes, num_features=num_features,
                                 use_bn=use_bn)
        self.logits = _BNReluConv(num_features, num_classes, batch_norm=use_bn)
